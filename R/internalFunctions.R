...
ORAperCluster <- function(Object, doORA, clusters, clus, completeRes = FALSE) {
  ...
  for (i in unique(clus)) {
    ...
    if (toupper(Object@metadata$structure[1]) == "SYMBOL") {
      genes2check <- toupper(genes2check)

      if (table(genes2check==toupper(genes2check))[1] < length(genes2check)/2 | table(genes2check==tolower(genes2check))[1] < length(genes2check)/2 | all(genes2check==toupper(genes2check)) | all(genes2check==tolower(genes2check)))
        { # To correct capital letters of gene symbol
        genes2check <- toupper(genes2check)
    ...
    ora <- clusterProfiler::enrichGO(na.omit(entrezID), OrgDb=usingOrg, keyType="ENTREZID",
                                     ont="BP", pvalueCutoff=0.05, pAdjustMethod="BH", qvalueCutoff=1)

    if (completeRes == TRUE) {
      keywords[[i]] <- ora@result
    } else {
...
prepare_plotting_df <- function(df) {
  ...
  plot_cluster_size_z <- aggregate(value ~ cluster,
                                   data = plot_melt_mtx_z, FUN = sum)
...
getGenesTissue <- function(Object, mol.unique.df, dic)
{
  ...
  if (substr(genes[1], 1, 3) == "ENS") {
    ...
    meltedtouse <- meltedtouse[which(meltedtouse$value==1),
                               c("variable", "ENSID")]
...
checkIndependent <- function(Object)
{
  ...
  {
    message("The cluster-independent/seriation-based method is not defined. Please firstly run ClusterIndependentGeneSet in order to select the ordering method.")
    stop()
  }
}

#checks if the cluster slot is null
checkDependent <- function(Object)
{
  if (is.null(Object@functionalAnnot))
  {
    message("The cluster-dependent/classic method is not defined. Please firstly run ClusterGeneSet in order to generate the clusters.")
    stop()
  }
}
...
PerformORAGOperCluster <- function(Object, uniquePathways, clusterIndependent, minNumber)
{

  go_id_list_new <- obtainGOidList(Object, uniquePathways, clusterIndependent, minNumber)
  #cluster-independent
  ...
  if (checkGO(Object) == FALSE) {
    ...
    # env_tdm_GO <- readRDS(system.file("extdata", "tdm_GO.rds", package = "simplifyEnrichment"))
    env_tdm_GO <- readRDS(system.file("extdata", "tdm_GO.rds", package = "GeneSetCluster"))
...
obtainGOidList <- function(Object, uniquePathways, clusterIndependent, minNumber)
{
  #cluster-independent
  if (clusterIndependent)
  {
...
obtainGREATtable <- function(GREATobject)
{
  ...
  great_1 <- rGREAT::getEnrichmentTable(GREATobject)
  zgreat_1 <- great_1[great_1$p_adjust<0.05,]
  pathways_1 <- great_1[, "id"]
  genes_1 <- rGREAT::getRegionGeneAssociations(GREATobject, use_symbols = FALSE)
  genes_1 <- unique(unlist(genes_1$annotated_genes))
  gene_sets1 <- data.frame(id=c(0), Genes=c(0))
  for(i in 1:length(GREATobject@gene_sets)){
    id <- names(GREATobject@gene_sets[i])
    genes <- GREATobject@gene_sets[[i]][which(GREATobject@gene_sets[[i]] %in% genes_1)]
...
optimalDist <- function(mat_sym, use_method=NULL) {
  set.seed(123)
